...
installed=false
  if [ -f /etc/profile.d/sqoop-$SQOOP_VERSION.sh ]; then
      source /etc/profile.d/sqoop-$SQOOP_VERSION.sh
      source $SQOOP_HOME/etc/sqoop/sqoop-env.sh
      installed=true
  fi
create_config()
{
...
# Organization: https://github.com/whaleai
"
#Configuring Environment of sqoop Daemons
SqoopEnv="
